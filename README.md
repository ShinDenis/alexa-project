# Amazon Alexa Reviews — Sentiment Analysis


___
## Web-demo. Запуск - [🚀 Live Demo](https://alexa-project.onrender.com)
___

Проект по классификации отзывов на устройства Amazon Alexa.

Бинарная классификация, определение характера отзыва: **Positive / Negative**.

Проект реализован в рамках ClassicML - моделирование с применением базовой техники Term Frequency, Inverse Document Frequency.

## Структура репозитория

```
amazon-alexa-sentiment/
├── app/
│   ├── app.py
│   ├── model.pkl
│   └── templates/
│       └── index.html
├── notebooks/
│   ├── Analysis.ipynb                  # EDA анализ
│   └── Feature_eng_Model_Results.ipynb # Инженерия, модели, результаты.
├── dataset/
│   └── amazon_alexa.tsv
├── render.yaml
├── requirements.txt
└── README.md
```

---
## Основные проблемы
 Дисбаланс выборки осложняет обучение модели.

## Ключевые решения
Акцент сделан на предсказании негативного класса.

- Двойной TF-IDF **NLP:** NLTK (стемминг + лемматизация) с сохранением слов-отрицаний.
- Feature Engineering:
  - обработаны пропуски и дубликаты,
  - добавлены переменные:
    - тип устройства `device_type`
    - длина отзыва`review_length`
- Тюнинг параметров с приоритетом на F1, recall негативного класса 
- Подбор порога принятия решений по F1 негативного класса (0.414 → 0.699)
- Выбрана модель LinearSVC с калибровкой вероятностей через `CalibratedClassifierCV`
---

## Результаты моделей

| Model                      | Test Accuracy | Test ROC-AUC | Test PR-AUC Negative | Best Threshold | Best Negative - precision | Best Negative - recall | Best Negative - f1-score |
|----------------------------|---------------|--------------|--------------------- |----------------|---------------------------|------------------------|--------------------------|
| LogisticRegression         | 0.914         | 0.947        | 0.648                | 0.47           | 0.571                     | 0.800                  | 0.667                    |
| **LinearSVC**              | 0.916         | 0.949        | 0.664                | 0.77           | **0.604**                 | **0.829**              | **0.699**                |
| DecisionTreeClassifier     | 0.926         | 0.928        | 0.609                | 0.54           | 0.500                     | 0.714                  | 0.588                    |
| GradientBoostingClassifier | 0.946         | 0.950        | 0.756                | 0.69           | 0.808                     | 0.600                  | 0.689                    |
| XGBClassifier              | 0.914         | 0.904        | 0.455                | 0.88           | 0.472                     | 0.486                  | 0.479                    |
| MultinomialNB              | 0.938         | 0.921        | 0.613                | 0.74           | 0.714                     | 0.429                  | 0.536                    |
| KNeighborsClassifier       | 0.919         | 0.792        | 0.398                | 0.82           | 0.381                     | 0.686                  | 0.490                    |
| StackingClassifier         | 0.862         | 0.955        | 0.685                | 0.21           | 0.622                     | 0.800                  | 0.700                    |

## Так как основной целью бизнеса является отслеживание и работа с негативными отзывами в целях улучшения качества продукта, выбор модели следующий:
### Лучшая модель — **LinearSVC** с калиброванными вероятностями и подобранным порогом 0.77.

---

# Итоги и рекомендации.

### В рамках проекта и выбранной методологии решения:
- была реализована задача выбора и построения оптимальной модели ClassicML.
- решена проблема дисбаланса классов.
- выбраны метрики для решения конкретной бизнесс задачи.
- подобраны лучшие параметры модели для максимизации метрик.
- достигнуты высокие показатели.
- интерпретированы результаты.

## Для улучшения качества и производительности можели рекомендуется использовать более продвинутые методы реализации.
- эмбеддинги - вектроные представления (пр: Word2Vec).
- векторы и нейросети (RNN, LSTM).
- трансформеры с attention, для учета контекста (Bert).
- большие языковые модели (LLM).

## В настоящее время производится реализация продвинутых техник решения задачи.
### Проект будет обновляться для показа эволюции методов и улучшения результатов.